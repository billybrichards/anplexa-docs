...
import { NatalChartData, type PlanetPlacement, type House, type AspectData } from '@anplexa/core/domain/value-objects/astrology/NatalChartData';
...
export class SimplifiedAstrologyService implements IAstrologyCalculationService {
  ...
  async calculateNatalChart(
    ...
    let midheaven: ZodiacSign | null = null;
  ...
  private createPlacement(planetName: string, position: PlanetPosition): PlanetPlacement {
  ...
  private calculatePlanetaryPositions(julianDay: number): Record<string, PlanetPosition> {
  ...
  private calculateHouses(julianDay: number, latitude: number, _longitude: number): number[] {
  ...
  private findHouse(degree: number, cusps: number[]): number {
    for (let i = 0; i < 12; i++) {
  ...
  private calculateAspects(planets: Record<string, PlanetPlacement>): AspectData[] {
        ...
        for (const aspectType of aspectTypes) {
          ...
          if (orb <= aspectType.orb) {
            aspects.push({
              planet1: name1,
              planet2: name2,
              aspectType: aspectType.name as any,
              orb,
  ...
  private calculateDominants(planets: Record<string, PlanetPlacement>): {
    ...
    const dominantModality = Object.entries(modalities)
      .sort(([, a], [, b]) => b - a)[0][0] as 'cardinal' | 'fixed' | 'mutable';
  ...
  async generateInterpretation(chartData: NatalChartData): Promise<string> {
  ...
  async generateCompanionContext(chartData: NatalChartData): Promise<string> {
    const big3 = chartData.getBigThree();
    const parts: string[] = [];
  ...
  private getSignDescription(sign: string, placement: 'sun' | 'moon' | 'rising'): string {
  ...
  private getCommunicationGuidance(element: string, moonSign: string): string {
  ...
  private getMoonSignNeed(moonSign: string): string {
