...
class UserRepository {
    ...
    async getById(id) {
        try {
            const result = await this.db
                ...
                .where((0, database_1.eq)(database_1.users.id, id))
                .limit(1);
    ...
    async getByEmail(email) {
        try {
            const result = await this.db
                .select()
                .from(database_1.users)
                .where((0, database_1.eq)(database_1.users.email, email))
                .limit(1);
            return result[0] || null;
        }
        catch (error) {
    ...
    async getAll(options) {
        try {
            let query = this.db.select().from(database_1.users);
    ...
    async create(userData) {
        try {
            ...
            // Insert user
            await this.db.insert(database_1.users).values(newUser);
    ...
    async update(id, updates) {
        try {
            ...
            const cleanedUpdateData = Object.fromEntries(Object.entries(updateData).filter(([_, value]) => value !== undefined));
            // Update user
            await this.db
    ...
    async delete(id) {
        try {
            ...
            // Delete user
            await this.db.delete(database_1.users).where((0, database_1.eq)(database_1.users.id, id));
        }
        catch (error) {
