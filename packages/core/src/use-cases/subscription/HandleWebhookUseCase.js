...
exports.HandleWebhookUseCase = exports.HandleWebhookUseCaseError = void 0;
const stripe_1 = require("@anplexa/services/stripe");
class HandleWebhookUseCaseError extends Error {
    code;
...
exports.HandleWebhookUseCaseError = HandleWebhookUseCaseError;
class HandleWebhookUseCase {
    ...
    async execute(request) {
        ...
        let event;
        try {
            event = (0, stripe_1.constructWebhookEvent)(request.payload, request.signature);
        }
        catch (error) {
            throw new HandleWebhookUseCaseError(`Webhook signature verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'INVALID_SIGNATURE');
        }
        ...
        try {
            ...
            throw new HandleWebhookUseCaseError(`Failed to process webhook event: ${error instanceof Error ? error.message : 'Unknown error'}`, 'PROCESSING_ERROR');
        }
    }
    /**
    ...
    async handleCheckoutSessionCompleted(session) {
        const sessionData = (0, stripe_1.handleCheckoutCompleted)(session);
    ...
    async handleSubscriptionCreated(subscription) {
        const subscriptionData = (0, stripe_1.handleSubscriptionCreated)(subscription);
        ...
        if (!user) {
            ...
            await this.userRepository.updateSubscriptionStatus(userId, subscriptionData.isActive ? 'subscribed' : 'not_subscribed', subscriptionData.customerId, subscriptionData.subscriptionId);
            return {
                processed: true,
                userId,
        ...
        // Update user with subscription
        await this.userRepository.updateSubscriptionStatus(user.id, subscriptionData.isActive ? 'subscribed' : 'not_subscribed', subscriptionData.customerId, subscriptionData.subscriptionId);
        return {
            processed: true,
            userId: user.id,
    ...
    async handleSubscriptionUpdated(subscription) {
        const subscriptionData = (0, stripe_1.handleSubscriptionUpdated)(subscription);
        // Find user by Stripe subscription ID
        const user = await this.userRepository.findByStripeSubscriptionId(subscriptionData.subscriptionId);
        if (!user) {
        ...
        await this.userRepository.updateSubscriptionStatus(user.id, status, subscriptionData.customerId, subscriptionData.subscriptionId);
        return {
            processed: true,
            userId: user.id,
    ...
    async handleSubscriptionDeleted(subscription) {
        const subscriptionData = (0, stripe_1.handleSubscriptionDeleted)(subscription);
        // Find user by Stripe subscription ID
        const user = await this.userRepository.findByStripeSubscriptionId(subscriptionData.subscriptionId);
        if (!user) {
        ...
        await this.userRepository.updateSubscriptionStatus(user.id, 'canceled', subscriptionData.customerId, subscriptionData.subscriptionId);
        return {
            processed: true,
            userId: user.id,
    ...
    async handleInvoicePaid(invoice) {
        const invoiceData = (0, stripe_1.handleInvoicePaid)(invoice);
    ...
    async handleInvoicePaymentFailed(invoice) {
        const invoiceData = (0, stripe_1.handleInvoicePaymentFailed)(invoice);
        // Only process if this is a subscription invoice
        if (!invoiceData.subscriptionId) {
            return {
                processed: false,
        ...
        await this.userRepository.updateSubscriptionStatus(user.id, 'past_due', invoiceData.customerId, invoiceData.subscriptionId);
        return {
            processed: true,
            userId: user.id,
